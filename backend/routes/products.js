...
router.post('/', async (req, res) => {
  const { name, category, price, quantity, low_stock_threshold } = req.body;
  ...
  try {
    const [result] = await pool.query(
      ...
      [name, category || 'General', price, quantity, low_stock_threshold || 5, req.user.userId]
    );
...
router.put('/:id', async (req, res) => {
  ...
  try {
    ...
    await pool.query(
      'UPDATE products SET name = ?, category = ?, price = ?, quantity = ?, low_stock_threshold = ? WHERE id = ?',
      [
        name ?? current[0].name,
